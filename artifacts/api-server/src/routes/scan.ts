...
router.get('/scan/:jobId/status', (req: Request, res: Response) => {
  ...
  res.json({
    ...
    latestScreenshots: job.latestScreenshots,
  });
});
    ...
    if (sb) {
       ...
       if (data && data.length > 0) {
         scans = data.map(d => ({
            ...
            targetUrl: d.target_url as string,
            scannedAt: d.scanned_at as string,
            totalBugs: (d.summary as any)?.totalBugs ?? 0,
            healthScore: (d.summary as any)?.healthScore ?? 0,
            pagesScanned: (d.summary as any)?.totalPages ?? 0,
         }));
         res.json({ scans });
         return;
       }
    ...
    for (const entry of entries) {
      try {
        ...
        scans.push({
          ...
          pagesScanned: report.totalPages || report.pagesScanned?.length || 0,
        });
      } catch {
