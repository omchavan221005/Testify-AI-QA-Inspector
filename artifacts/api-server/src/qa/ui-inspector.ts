...
export async function runPageUIChecks(page: Page, url: string): Promise<UIIssue[]> {
  const rawIssues: RawIssue[] = await page.evaluate((pageUrl: string) => {
    ...
    (() => {
      ...
      let overlapCount = 0;
      for (let i = 0; i < rects.length && overlapCount < 10; i++) {
        for (let j = i + 1; j < rects.length && overlapCount < 10; j++) {
          ...
          if (overlap) {
            found.push({
              ...
              description: `Elements <${rects[i].el.tagName.toLowerCase()}> and <${rects[j].el.tagName.toLowerCase()}> overlap.`,
              impact:
                'When clickable elements overlap, users may accidentally trigger the wrong action, especially on mobile/touch interfaces.',
              recommendation:
                'Assign unique positions or add margins to ensure interactive elements are clearly separated.',
              selector: getCssPath(rects[i].el),
            });
            overlapCount++;
          }
        }
