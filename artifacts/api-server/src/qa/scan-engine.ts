...
import { getAIDebugInsight } from './ai-debugger.js';
import {
...
export async function runScan(job: ScanJob): Promise<void> {
  ...
  try {
    ...
    const { pages, allLinks, brokenResources } = await crawlSite(
      ...
      screenshotsDir,
      job.device,
      (currentUrl, progress, screenshot) => { 
        job.currentUrl = currentUrl;
        job.progress = Math.max(job.progress, progress);
        if (screenshot) {
          const list = job.latestScreenshots || [];
          if (!list.includes(screenshot)) {
            job.latestScreenshots = [...list, screenshot].slice(-5); // Keep the last 5
          }
        }
      }
    );
    ...
    // Map broken resources to broken links so they can be annotated
    ...
    // ── Annotate screenshots with red boxes (blocking) ──────────────
    job.currentStep = 'Annotating Error Screenshots...';
    await annotatePageScreenshots(pages, uiIssues, formIssues, [...brokenLinks, ...resourceBrokenLinks], screenshotsDir).catch((e) => console.error(e));
    if (job.cancelled) return;
    ...
    if (enableAI) {
      await Promise.all([
        ...brokenLinks.map(async (link) => {
          ...
          link.ai = await getAIDebugInsight({ type: 'Broken Link', message: link.linkUrl }, enableAI, openaiApiKey, aiModel);
        }),
        ...uiIssues.map(async (issue) => {
          ...
          issue.ai = await getAIDebugInsight({ type: issue.issueType, message: issue.description, selector: issue.selector }, enableAI, openaiApiKey, aiModel);
        }),
        ...formIssues.map(async (issue) => {
          const r = await classifyBug(issue.description, issue.issueType, enableAI, openaiApiKey, aiModel);
          issue.aiCategory = r.category;
          issue.aiConfidence = r.confidence;
          issue.ai = await getAIDebugInsight({ type: issue.issueType, message: issue.description, selector: issue.formSelector }, enableAI, openaiApiKey, aiModel);
        }),
      ]);
    } else {
      await Promise.all([
        ...brokenLinks.map(async (link) => {
          ...
          link.aiCategory = r.category;
          link.ai = await getAIDebugInsight({ type: 'Broken Link', message: link.linkUrl }, false);
        }),
        ...uiIssues.map(async (issue) => {
          const r = await classifyBug(issue.description, issue.issueType, false);
          issue.aiCategory = r.category;
          issue.ai = await getAIDebugInsight({ type: issue.issueType, message: issue.description, selector: issue.selector }, false);
        }),
        ...formIssues.map(async (issue) => {
          const r = await classifyBug(issue.description, issue.issueType, false);
          issue.aiCategory = r.category;
          issue.ai = await getAIDebugInsight({ type: issue.issueType, message: issue.description, selector: issue.formSelector }, false);
        }),
      ]);
    }
